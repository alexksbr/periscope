...
async def execute_tool_with_policy(
    *,
    ...
    call_recorder = recorder or NoopToolCallRecorder()
    call_tracer = tracer or trace.get_tracer(__name__)

    with call_tracer.start_as_current_span("periscope.tool.execute") as span:
        _set_initial_span_attributes(span, tool.name, context)
        try:
            result, normalized_input = await _execute_tool_once(
                tool=tool,
                arguments=arguments,
                context=context,
        ...
        await _record_result(call_recorder, context, result, normalized_input)
        return result
